...
int main ()
{
  ...
  ofstream file("results/uncertainty/syst_stat_error"+b_mu+"_"+version+".txt");
...
*/
    ...
    statsyst2[2] = calcul(1.0, 1.0, 1.0-per[2], 1.0-per[3], 1.0, 1.0);//single t dd
...
file << "statical error of b_mu when delta Chi^2 at 1 GeV = "<< stat <<endl;

cout << "total systematic error of b mu : " << sqrt(squaredSum);
file << "total systematic error of b mu : " << sqrt(squaredSum);
