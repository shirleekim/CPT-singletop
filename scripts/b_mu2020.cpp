...
string nameFile2 = "results/modulation/ft_"+b_mu+version+".root";
TLorentzVector b_mu0 (b_muvalue, 0., 0., 0.);//x
TLorentzVector b_mu1 (0., b_muvalue, 0., 0.); //y  //test
TLorentzVector b_mu2 (0., 0., b_muvalue, 0.); //z //test
TLorentzVector b_mu3 (0., 0., 0., b_muvalue); //t

TFile* File = new TFile (nameFile.c_str(), "UPDATE");
TFile* File2 = new TFile (nameFile2.c_str(),"UPDATE");
...
{// WeightAverage
  ...
  //cout<< "f(t) x : " << ft << endl;
  if (ft_x>max) {do {max += 0.01;} while (ft_x>max);};
  if (ft_x<min) {do {min -=0.01;} while (ft_x<min);};

  ft_y = b_mu1[1]*((sin(w*t)*sl_sth+cos(w*t)*cth)*WtAv[0]-(sin(w*t)*cl)*WtAv[1]+(sin(w*t)*sl_cth+cos(w*t)*sth)*WtAv[2]);
  graphy->SetPoint(i, t, ft_y);
  if (ft_y>max) {do {max += 0.01;} while (ft_y>max);};
  if (ft_y<min) {do {min -=0.01;} while (ft_y<min);};

  ft_z = -b_mu2[2]*((cl_sth)*WtAv[0]+(sl)*WtAv[2]+(cl_cth)*WtAv[2]);
  graphz->SetPoint(i, t, ft_z);
  if (ft_z>max) {do {max += 0.01;} while (ft_z>max);};
  if (ft_z<min) {do {min -=0.01;} while (ft_z<min);};
  //cout << "ft_z : " << ft_z << endl;

  ft_t = b_mu3[3]*WtAv[3];
 grapht->SetPoint(i, t, ft_t);
  if (ft_t>max) {do {max += 0.01;} while (ft_t>max);};
  if (ft_t<min) {do {min -=0.01;} while (ft_t<min);};
 //cout << "ft_t : " << ft_t << endl;


  ft = ft_t+ft_x+ft_y+ft_z;
  graphft->SetPoint(i, t, ft);
